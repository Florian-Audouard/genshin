...
function rhex(n) {
	...
	for (j = 0; j < 4; j++)
		s +=
			("0" + ((n >> (j * 8 + 4)) & 0x0f).toString(16)).slice(-2) +
...
function buildHeaders(ltokenV2, ltuidV2) {
	HEADERS["DS"] = generateDS();
	HEADERS["Cookie"] = `ltoken_v2=${ltokenV2}; ltuid_v2=${ltuidV2};`;
	return HEADERS;
}
async function fetchDiaryDetail(
	uid,
	type,
	headers,
    current_page = 1,
	...
	return await fetch(`${URL_DIARY_DETAIL}?${params.toString()}`, {
		method: "GET",
		headers,
	}).then((res) => res.json());
}

async function getDailyRewards(uid, ltokenV2, ltuidV2, rewardType) {
	// type accepted: 1 = primogem or 2 = mora
	const headers = buildHeaders(ltokenV2, ltuidV2);
	const now = new Date();
	...
	// Create "tomorrow 4 AM"
	const tomorrow4AM = new Date(today4AM);
	tomorrow4AM.setDate(today4AM.getDate() + 1);
	last_fetched_date = null;
	let diary = [];
    let current_page = 1;
	while (last_fetched_date === null || last_fetched_date >= today4AM) {
		const diaryTMP = (await fetchDiaryDetail(uid, rewardType, headers, current_page)).data
			.list;
		diary = diary.concat(diaryTMP);
		last_fetched_date = new Date(diaryTMP[diaryTMP.length - 1].time);
		current_page++;
	}
	console.log(diary[0]);
	...
	const filtered = diary.filter((item) => {
		...
		return itemTime >= today4AM && itemTime < tomorrow4AM;
	});
	// console.log(filtered);
	let count = 0;
	filtered.forEach((item) => (count += item.num));
	return count;
}
