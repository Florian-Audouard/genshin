...
abstract class Banner {
  ...
  pull(): boolean {
    ...
    if (this.calc_proba()) {
      this.current_pull = 0;
      result = this.calc_proba_when_5_star() || this.garentee;
...
pack.addFormula({
  name: "GetProba",
  description: "Get probability (%) of obtaining desired 5-star characters and weapons.",
  parameters: [
    coda.makeParameter({ type: coda.ParameterType.Number, name: "number_of_wishes", description: "Number of wishes" }),
    coda.makeParameter({ type: coda.ParameterType.Number, name: "number_of_5_stars_char", description: "Desired 5-star characters" }),
    coda.makeParameter({ type: coda.ParameterType.Number, name: "number_of_5_stars_weapon", description: "Desired 5-star weapons" }),
    coda.makeParameter({ type: coda.ParameterType.Number, name: "loose_streak_char_5_stars", description: "Current lose streak (character banner)", optional: true }),
    coda.makeParameter({ type: coda.ParameterType.Number, name: "initial_pity_char", description: "Initial pity (character banner)", optional: true }),
    coda.makeParameter({ type: coda.ParameterType.Number, name: "initial_pity_weapon", description: "Initial pity (weapon banner)", optional: true }),
    coda.makeParameter({ type: coda.ParameterType.Number, name: "number_of_pulls", description: "Number of Monte Carlo simulations", optional: true }),
  ],
  resultType: coda.ValueType.Number,
  execute: async function (
    [number_of_wishes, number_of_5_stars_char, number_of_5_stars_weapon, loose_streak_char_5_stars = 0, initial_pity_char = 0, initial_pity_weapon = 0, number_of_pulls = 100000]:
